...
/**
 * One icon family, one stroke weight, one grid — Lucide paths inlined so the
 * whole UI stays visually consistent and ships without an icon runtime.
 */
const ICONS: Record<string, string> = {
  ...
  needle: '<path d="M2 22l1-1"/><path d="M11.9 3.1 20.9 12.1"/><path d="m4 20 9.5-9.5"/><path d="M13.5 6.5 17 3l4 4-3.5 3.5"/><path d="M9 15l-2 2"/>',
...
export class IconComponent {
  ...
  readonly path = computed<SafeHtml>(() =>
    this.sanitizer.bypassSecurityTrustHtml(ICONS[this.name()] ?? ICONS['circle-dot']),
  );
}
